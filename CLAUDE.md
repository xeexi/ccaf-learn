...
| 段階 | px | 用途 |
|---|---|---|
| `--fs-xs` | 12 | ラベル・チップ・キャプション |
| `--fs-sm` | 14 | 補助テキスト・注記・小項目・コード（`code` と `pre.code`） |
| `--fs-base` | 16 | **基準。** ヘッダーのナビ・目次リンク・本文・小見出し（`h3.sub`）・箱の中・ボタン・入力 |
| `--fs-lg` | 26 | セクション見出し（`.sec-head h2` / `.hero h2` / `.card h2`） |
| `--fs-xl` | 32 | ページ見出し（`.hero h1`） |

**基準はヘッダーのナビと目次リンクのサイズ。本文もそれと同じ。**
本文がヘッダーより大きく見えたら、どこかで基準以外を当てている。

段階の外の値を作る書き方が3つある。**すべて `check.mjs` が落とす。**

1. `font-size` の px 直書き（SVG の中だけは別系統）
2. `clamp()` ── 幅に応じた中間サイズを無限に作る。`.hero h1` に入っていて外した
3. `em` / `rem` / `%` ── 親のサイズぶんだけ段階外の値ができる。
   `code{ font-size:.86em }` が **12.04px と 13.76px を実際に画面に出していた**。
   `--fs-*` を grep するだけでは見つからず、**ブラウザで `getComputedStyle` を集計して発覚した**
...
トップページには目次がないので、`.hero` と `.cards` に `max-width:1052px`（1000＋余白26×2）を当てて
同じ列幅にしている。

**個々の要素に `max-width` を当てて狭めないこと。** `.hero p` に `max-width:48em` が入っていたせいで、
段落だけ比較表や図より 400px 手前で折り返し、改行位置が中途半端に見えていた。
しかも `em` なので**本文を縮めると読み幅まで一緒に縮む**。幅は容器側で1回だけ決める。

### 見た目はブラウザで測る

**Claude が確認する場合は headless Chromium を使うこと。** 今日見つけた不具合の大半は、
目視ではなく**測定**で見つかっている。

サンドボックスには Chromium がなく、`npx playwright install` は**ダウンロードに失敗する**
（`cdn.playwright.dev` に届かない）。npm レジストリは通るので、バイナリを同梱している
`@sparticuz/chromium` から実行ファイルを取って Playwright に渡す：

```js
import { chromium } from 'playwright';
import sp from '@sparticuz/chromium';          // CommonJS 経由なら (sp.default||sp)
...
③390 / 640 / 860 / 1024 / 1440px で**横溢れと図中の実効文字サイズ**
④**両テーマの色コントラスト**（WCAG AA ─ 通常 4.5:1、24px 以上または太字 18.66px 以上は 3:1）。

コントラストを測るときの注意が3つある。**3つとも実際に誤った結果を出した。**

1. `color-mix()` や `color()` は **0〜1 の小数**で返る（`color(srgb 0.96 0.97 0.98 / 0.86)`）。
   0〜255 と決めつけない。**canvas に描いて `getImageData` で読む**のが確実
2. 半透明の地色は**祖先をたどって合成する**。1つ見つけて止めると値が狂う
3. `transition` が効いていると**遷移中の中間色**を拾う。
   `*{transition:none !important}` を注入してから切り替える
...
| 15 | 「段階は7つ」と CLAUDE.md に書いてあったが、**実際に画面に出ていたのは7種類ではなかった。** `--fs-mono` を数え落としていて定義は8つ、さらに `code{ font-size:.86em }` が **12.04px と 13.76px** を、`.hero h1` の `clamp()` が中間サイズを作っていた | **トークンを grep して数えるだけでは足りない。** `getComputedStyle` の `fontSize` を全ページの全要素で集計して、**画面に出ている実測値の種類**を数える。`em` / `%` / `clamp()` は `check.mjs` が落とす |
| 16 | `.hero p` にだけ `max-width:48em` が付いていて、段落が比較表・図より **400px 手前で折り返していた**。しかも `em` なので本文を縮めると読み幅も一緒に縮む | **読み幅は容器側で1回だけ決める。** 個々の要素に `max-width` を当てない。右端が揃っているかは `getBoundingClientRect().right` を集めて確かめる |
| 17 | ライトテーマで**アクセント色だけ上書きし忘れて**いた。ダーク用の明るい7色（`#22D3EE` など）が白地にそのまま乗り、**コントラスト 1.0〜1.8:1**。目次見出し・`BLUEPRINT` ラベル・「図の読み方」など2000か所以上。さらに `color-mix(var(--amber) 20%, transparent)` という**「濃い色を混ぜて地色を作る」書き方がライトでは地を暗くして**いた（ダークでは明るくなる） | **テーマを足したら、変数の集合を突き合わせて漏れを探す。** 色の設計は「明るい／暗い」ではなく**地に対する比**で決まるので、`color-mix` で作る地色は**両テーマで向きが逆になる**ことを疑う。確認は全ページのコントラスト実測（§3） |
| 18 | 「`app.js` がテーマを復元していない」と報告したが**誤り**だった。復元は `reindex.mjs` の `THEMEBOOT`（`<body>` 直下のインラインスクリプト）が担っていて、正常に動いていた | 1つのファイルを見ただけで「機能がない」と言わない。**動かして確かめる**（ボタンを押す → 別ページへ移動 → 属性を読む） |
...
- **文字サイズを5段階に整理**（§3）。基準はヘッダー・目次のサイズで、本文もそれと同じ
- **トップページの読み幅を節ページと同じ 1000px に揃えた**（§3）
- **両テーマの色コントラストを全89ページで実測**し、WCAG AA 未達を0にした（§3・§7 #17）

...
/* --- 5w. 正解だけが長くなっていないか --------------------------------
   §7 #29 で直した「正解が2番目に偏る」と同じ形の癖が、**長さ**にも出る。
   正解には理由まで書き、誤答は短く切り捨てるので、正解が独りでに長くなる。
   混ぜても長さは動かないので、位置の偏りより強い手がかりになる。
   直す向きは「選択肢には選ぶものだけを書き、理由は解説（e）へ移す」。 */
console.log('\n■ 正解だけが長くないか');
let olNg = 0, olTop = 0;
Object.entries(QUIZ).forEach(([k, v]) => v.forEach((x, i) => {
  const L = x.o.map(o => o.replace(/<[^>]+>/g, '').length);
  const ng = Math.max(...L.filter((_, j) => j !== x.a));
  if (L[x.a] === Math.max(...L)) olTop++;
  if (L[x.a] > ng * 1.5) {
    bad(`設問キー ${k} Q${i + 1}: 正解 ${L[x.a]}字 ÷ 他の最長 ${ng}字 = ${(L[x.a] / ng).toFixed(2)}倍（1.5倍まで。理由を解説へ移す）`);
    olNg++;
  }
}));
